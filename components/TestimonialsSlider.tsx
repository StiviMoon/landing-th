...
const testimonials = [
  ...
  return (
    <section className="py-16 md:py-20 bg-light overflow-hidden">
          ...
          <div className="card p-4 md:p-6 lg:p-8 text-center shadow-xl">
            <div className="flex justify-center mb-3 md:mb-4">
              {[...Array(testimonials[currentSlide].rating)].map((_, i) => (
                <Star key={i} className="h-4 w-4 md:h-5 md:w-5 text-gold fill-current" />
              ))}
            </div>
            ...
            <div>
              <p className="font-bold text-primary text-base md:text-lg leading-tight">{testimonials[currentSlide].name}</p>
              <p className="text-muted-foreground text-sm md:text-base">{testimonials[currentSlide].role}</p>
            </div>
          </div>
